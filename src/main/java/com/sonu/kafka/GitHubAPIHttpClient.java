...
public class GitHubAPIHttpClient {
    ...
    private static final Logger log = LoggerFactory.getLogger(GitHubAPIHttpClient.class.getSimpleName());
    ...
    protected JSONArray getNextIssues(Integer page, Instant since) throws InterruptedException, ConnectException {
        HttpResponse<JsonNode> jsonResponse;
        try {
            jsonResponse = getNextIssuesAPI(page,since);
            //deal with headers
            Headers headers = jsonResponse.getHeaders();
            if(headers.getFirst("X-RateLimit-Limit") != null ){
                XRateLimit = Integer.valueOf(headers.getFirst("X-RateLimit-Limit"));

            }
            if(headers.getFirst("X-RateLimit-Remaining") != null ) {
                XRateRemaining = Integer.valueOf(headers.getFirst("X-RateLimit-Remaining"));

            }
            if(headers.getFirst("X-RateLimit-Reset") != null ) {
                XRateReset = Long.valueOf(headers.getFirst("X-RateLimit-Reset"));
            ...
            switch (jsonResponse.getStatus()){
                ...
                case 403:
                    ...
                    log.info(String.format("Your limit will reset at %s",
                ...
                default:
                    ...
                    log.error("Unknown Error: sleeping for 5 seconds " +
                            "before retry");
                    Thread.sleep(5000L);
                    return getNextIssues(page, since);
            }
    ...
    protected HttpResponse<JsonNode> getNextIssuesAPI(Integer page, Instant since) throws UnirestException {
        ...
        if(!(config.getAuthUsernameConfig().isEmpty()) && !(config.getAuthPasswordConfig().isEmpty())){
            unirest.basicAuth(config.getAuthUsernameConfig(),config.getAuthPasswordConfig());
        }
    ...
    protected String constructUrl(Integer page, Instant since) {
        return String.format("https://api.github.com/repos/%s/%s/issues?page=%s&per_page=%s&since=%s&state=all&direction=asc&sort=updated",
